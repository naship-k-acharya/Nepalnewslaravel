...
<div class="post-container">
    ...
    <form action="{{ route('ads.store') }}" method="POST" enctype="multipart/form-data">
        ...
        <div class="form-group">
            ...
            <input type="file" id="photo" name="image" class="form-control-file" accept="image/*" required>
        </div>
        <div class="form-group">
            <label for="title" class="form-label">link</label>
            <input type="text" id="title" name="link" class="form-control" required>
        </div>
    ...
    <ul >
        ...
        @foreach($ads as $singlePost)
            <li style="margin-top: 10px; background-color:#ccc;" class="post-item">
                <a href="{{ $singlePost->link }}" class="ad-link">{{ $singlePost->link }}</a>
                <form action="{{ route('add.destroy', $singlePost->id) }}" method="POST" class="delete-form">
                    @csrf
                    @method('DELETE')
